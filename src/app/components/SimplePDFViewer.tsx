            ...
            <CardContent className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden bg-gray-50 p-4">
                <div className="flex flex-wrap items-center gap-3 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                        ...
                        <Button type="button" size="sm" onClick={exportAnnotatedPdf} disabled={isLoading || isExporting || !pdfDocument}>
                ...
                <div ref={wrapperRef} className="flex-1 min-h-0 overflow-auto rounded-xl border border-gray-200 bg-white p-4 scroll-smooth snap-y snap-mandatory">
                    {isLoading && (
                    ...
                    {!isLoading && !error && pageCount > 0 && (
                        <div className="space-y-6">
                            {Array.from({ length: pageCount }, (_, index) => index + 1).map((pageNumber) => (
                                <div key={pageNumber} className="snap-start overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                                    <div className="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-3 py-2 text-xs font-medium text-gray-600">
                                        ...
                                        <span>{pageMetricsRef.current[pageNumber]?.scale ? `${Math.round(pageMetricsRef.current[pageNumber].scale * 100)}%` : 'Rendering...'}</span>
                                    </div>
