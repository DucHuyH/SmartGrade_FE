...
const mapChatMessagesToView = (messagesList: DirectChatMessage[], currentUserId: number | null): MessageView[] => {
  return messagesList.map((message) => ({
    id: message.id,
    sender: message.senderId === currentUserId ? 'student' : 'lecturer',
        ...
        >
          ...
          <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
            <DialogHeader>
              ...
              <DialogDescription>Send a message to your lecturer about a course or assignment</DialogDescription>
            </DialogHeader>
                    ...
                    <div className='space-y-3'>
                      <div className='border border-blue-200 bg-blue-50 rounded-lg p-3'>
                        <div className='flex items-start gap-3'>
                          ...
                          <div className='flex-1 min-w-0'>
                            ...
                            <p className='text-sm text-gray-600'>Selected assignment loaded from API</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {selectedAssignmentForCompose && (
                        <div
                          ...
                          onClick={() => setAttachSubmission(!attachSubmission)}
                        >
                          <Checkbox
                ...
                {attachSubmission && selectedAssignmentForCompose && (
                  <div className='flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg'>
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      <p className='text-xs text-green-700'>This follows the selected assignment from the API.</p>
                    </div>
                    <button
                      type='button'
                      onClick={() => setAttachSubmission(false)}
                      className='text-green-700 hover:text-green-900'
                      title='Remove attached submission'
                    >
                      <X className='h-4 w-4' />
                    </button>
                  </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={isComposeSending || !composeMessage.trim() || !composeCourseId}
              >
                {isComposeSending ? (
                  <>
                ...
                <div className='space-y-3 mt-4'>
                  <div className='relative'>
                    ...
                    <Input
                      ...
                      className='pl-9'
                      aria-label='Search conversations with lecturers'
                    />
                  </div>
                      ...
                      <div className='shrink-0 w-48'>
                        {selectedConv?.hasSubmission ? (
                          selectedConv?.isPublished ? (
                            <Link to={`/student/grades/${selectedConv?.submissionId || 'SUB001'}`} className='block'>
                              <div className='border border-blue-200 bg-blue-50 rounded-lg p-2.5 hover:bg-blue-100 transition-colors'>
                                <div className='flex items-center gap-2'>
                                  ...
                                  <div className='flex-1 min-w-0'>
                                    ...
                                    <p className='text-xs text-blue-600 mt-0.5'>Click to view grade</p>
                                  </div>
                                </div>
                              </div>
                            </Link>
                            ...
                            <div className='border border-yellow-200 bg-yellow-50 rounded-lg p-2.5'>
                              <div className='flex items-center gap-2'>
                                ...
                                <div className='flex-1 min-w-0'>
                                  <p className='text-xs text-yellow-800 font-medium'>Grading in progress</p>
                                  <p className='text-xs text-yellow-600 mt-0.5'>Check back later</p>
                                </div>
                              </div>
                            </div>
                          )
                    ...
                    <CardContent>
                              ...
                              >
                                <div
                                  className={`max-w-[70%] rounded-lg p-3 ${
                                    message.sender === 'student' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-900'
                                  }`}
                                >
                      ...
                      <div className='space-y-3'>
                        ...
                        {selectedConv?.hasSubmission && (
                          <div
                            className='flex items-start gap-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer'
                            onClick={() => setAttachSubmissionInChat(!attachSubmissionInChat)}
                          >
                            <Checkbox
                    ...
                    {filteredCourseAnnouncements.length > 0 ? (
                      filteredCourseAnnouncements.map((course) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedCourseAnnouncement === course.id ? 'bg-primary text-white' : 'hover:bg-gray-100'
                          }`}
                        >
                          <div className='flex justify-between items-start mb-1'>
                            <span
                              className={`font-medium ${selectedCourseAnnouncement === course.id ? '' : 'text-sm'}`}
                            >
                              {course.courseName}
                            </span>
                            {course.unread > 0 && (
                          ...
                          <p
                            className={`text-xs mb-1 ${
                              selectedCourseAnnouncement === course.id ? 'text-white/80' : 'text-gray-500'
                            }`}
                          >
                          ...
                          <p
                            className={`text-xs mb-1 ${
                              selectedCourseAnnouncement === course.id ? 'text-white/70' : 'text-gray-600'
                            }`}
                          >
                            {course.semester}
                          </p>
                          <p
                            className={`text-xs truncate mb-1 ${
                              selectedCourseAnnouncement === course.id ? 'text-white/80' : 'text-gray-600'
                            }`}
                          >
                            {course.lastMessage}
                          </p>
                          <span
                            className={`text-xs ${
                              selectedCourseAnnouncement === course.id ? 'text-white/60' : 'text-gray-400'
                            }`}
                          >
                            {course.time}
                          </span>
                  ...
                  <CardHeader>
                    <div className='flex items-start justify-between gap-4'>
                      <div className='flex-1 min-w-0'>
                        ...
                        <p className='text-sm text-gray-600'>
                          {selectedCourseAnn?.courseId} - {selectedCourseAnn?.semester} -{' '}
                          {selectedCourseAnn?.academicYear}
                        </p>
                        <p className='text-sm text-gray-700 mt-1'>Lecturer: {selectedCourseAnn?.lecturer}</p>
                      </div>
                    </div>
                  </CardHeader>
                </>
              ...
              <CardContent>
                  ...
                  <div className='space-y-4'>
                    {currentAnnouncementMessages.length > 0 ? (
                      ...
                      <div className='text-center py-12 text-gray-500'>
                        ...
                        <p className='text-sm text-gray-400 mt-1'>Course announcements will appear here</p>
                      </div>
                    )}
                  </div>
                </ScrollArea>
                ...
                <div className='bg-blue-50 rounded-lg border border-blue-200 p-3'>
                  <p className='text-xs text-blue-800'>
                    <span className='font-medium'>Note:</span> This is a read-only announcement channel. To communicate
                    with your lecturer, please use the Direct Messages tab or the messaging feature in your assignment
                    grade view.
                  </p>
                </div>
              </CardContent>
