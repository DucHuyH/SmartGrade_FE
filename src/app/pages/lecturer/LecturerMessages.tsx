...
const mapChatMessagesToView = (messagesList: DirectChatMessage[], currentUserId: number | null): MessageView[] => {
  return messagesList.map((message) => ({
    id: message.id,
    sender: message.senderId === currentUserId ? 'lecturer' : 'student',
...
export function LecturerMessages() {
  ...
  const lecturerTemplates = {
    regrade:
      'I have already regraded your submission and provided updated feedback. Please check your grade page to view the new grade and detailed comments. If you have any questions about the grading, feel free to reach out.',
    feedback:
      'I have added additional feedback to your submission. Please review the comments on your grade page carefully, as they contain important insights to help you improve in future assignments.',
    deadline:
      'I have reviewed your request for a deadline extension. Based on the circumstances you described, I am granting you an extension until [DATE]. Please ensure your submission is complete and submitted by the new deadline.',
    office_hours:
      'I appreciate your questions about this assignment. I think it would be beneficial to discuss these topics in more detail during office hours. My office hours are [DAYS/TIMES]. Please feel free to stop by or let me know if you need to schedule a different time.',
    clarification:
      "Thank you for reaching out. Let me clarify the requirements for this assignment: [EXPLANATION]. I hope this helps! If you need further clarification, please don't hesitate to ask.",
    encouragement:
      "I reviewed your submission and can see you put significant effort into this work. While there are areas for improvement, you're making good progress. Keep up the good work, and don't hesitate to ask if you need help with future assignments."
  }
  ...
  const handleSend = () => {
    ...
    sendChatMessage(
      {
        course_id: selectedConv.courseId,
  ...
  const handleSendComposedMessage = async () => {
    ...
    try {
      ...
      if (messageType === 'one-to-one') {
        ...
        const studentIds = selectedStudents.map((id) => toNumericId(id)).filter((id): id is number => id !== null)
        ...
        >
          ...
          <DialogContent className='max-w-3xl max-h-[90vh] overflow-y-auto'>
            <DialogHeader>
              ...
              <DialogDescription>Send a message to individual students, groups, or entire courses</DialogDescription>
            </DialogHeader>
                ...
                <div className='space-y-3'>
                  ...
                  <div className='relative mb-3'>
                    ...
                    <Input
                      ...
                      className='pl-9'
                      aria-label='Search students for one-to-one message'
                    />
                  </div>
                      ...
                      {filteredStudentsForCompose.map((student) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedStudent === student.id ? 'bg-primary text-white' : 'hover:bg-gray-100'
                          }`}
                        >
                          <div className='text-sm font-medium'>{student.name}</div>
                          <div
                            className={`text-xs ${selectedStudent === student.id ? 'text-white/70' : 'text-gray-500'}`}
                          >
                ...
                <div className='space-y-3'>
                  ...
                  <div className='relative mb-3'>
                    ...
                    <Input
                      ...
                      onChange={(e) => setComposeSearchQuery(e.target.value)}
                      className='pl-9'
                      aria-label='Search students for group message'
                    />
                  </div>

                  <div className='flex items-center justify-between'>
                    <Button type='button' variant='outline' size='sm' onClick={handleSelectAllStudents}>
                      {selectedStudents.length === filteredStudentsForCompose.length ? 'Deselect All' : 'Select All'}
                    </Button>
              ...
              {messageType === 'group' && selectedStudents.length > 1 && (
                <div className='bg-blue-50 border border-blue-200 rounded-lg p-3'>
                  <p className='text-xs text-blue-800'>
                    <span className='font-medium'>Note:</span> Sending to {selectedStudents.length} students will take
                    approximately {((selectedStudents.length - 1) * 350) / 1000}s due to rate limiting.
                  </p>
                </div>
              )}
            </div>
                    ...
                    {filteredConversations.length > 0 ? (
                      filteredConversations.map((conversation) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedConversation === conversation.id ? 'bg-primary text-white' : 'hover:bg-gray-100'
                          }`}
                        >
                          <div className='flex justify-between items-start mb-1'>
                            <span
                              className={`font-medium ${selectedConversation === conversation.id ? '' : 'text-sm'}`}
                            >
                    ...
                    <div className='flex items-start justify-between gap-4'>
                      <div className='flex-1 min-w-0'>
                        <CardTitle className='mb-2'>
                          {peerName || selectedConv?.student} - {selectedConv?.studentId}
                        </CardTitle>
                        <p className='text-sm text-gray-600'>
                          {selectedConv?.courseName} - {selectedConv?.courseId} - {selectedConv?.semester} -{' '}
                          {selectedConv?.academicYear}
                        </p>
                        <p className='text-sm text-gray-700 mt-1'>{selectedConv?.assignmentName}</p>
                      </div>
                        ...
                        {selectedConv?.hasSubmission ? (
                              ...
                              <div className='border border-blue-200 bg-blue-50 rounded-lg p-2.5 hover:bg-blue-100 transition-colors'>
                                <div className='flex items-center gap-2'>
                                  ...
                                  <div className='flex-1 min-w-0'>
                                    ...
                                    <p className='text-xs text-blue-600 mt-0.5'>Click to view & regrade</p>
                                  </div>
                                </div>
                              </div>
                            </Link>
                              ...
                              <div className='border border-yellow-200 bg-yellow-50 rounded-lg p-2.5 hover:bg-yellow-100 transition-colors'>
                                <div className='flex items-center gap-2'>
                                  ...
                                  <div className='flex-1 min-w-0'>
                                    ...
                                    <p className='text-xs text-yellow-600 mt-0.5'>Graded, not published</p>
                                  </div>
                                </div>
                              </div>
                            </Link>
                          ...
                          <div className='border border-gray-200 bg-gray-50 rounded-lg p-2.5'>
                            <div className='flex items-center gap-2'>
                              ...
                              <p className='text-xs text-gray-600'>No submission available</p>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <CardContent>
                      ...
                      <div className='space-y-3'>
                        ...
                        <div className='bg-blue-50 rounded-lg border border-blue-200 p-3'>
                          <label className='text-xs text-gray-700 mb-2 block'>Message Templates</label>
                    ...
                    {filteredCourseConversations.length > 0 ? (
                      filteredCourseConversations.map((course) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-lg transition-colors ${
                            selectedCourseConversation === course.id ? 'bg-primary text-white' : 'hover:bg-gray-100'
                          }`}
                        >
                          <div className='flex justify-between items-start mb-1'>
                            <span
                              className={`font-medium ${selectedCourseConversation === course.id ? '' : 'text-sm'}`}
                            >
                              {course.courseName}
                            </span>
                          </div>
                          ...
                          <div className='flex items-center gap-2 mb-1'>
                            <Users
                              className={`h-3 w-3 ${selectedCourseConversation === course.id ? 'text-white/70' : 'text-gray-500'}`}
                            />
                            <p
                              className={`text-xs ${
                  ...
                  <CardHeader>
                    <div className='flex items-start justify-between gap-4'>
                      <div className='flex-1 min-w-0'>
                        ...
                        <p className='text-sm text-gray-600'>
                          {selectedCourseConv?.courseId} - {selectedCourseConv?.semester} -{' '}
                          {selectedCourseConv?.academicYear}
                        </p>
                  ...
                  <div className='space-y-3'>
                    <div className='bg-blue-50 rounded-lg border border-blue-200 p-3'>
                      <p className='text-xs text-blue-800 mb-1'>
                        <span className='font-medium'>Course-wide Broadcast:</span> Your message will be sent to all{' '}
                        {selectedCourseConv?.studentCount || 0} students in this course
                      </p>
                    </div>
